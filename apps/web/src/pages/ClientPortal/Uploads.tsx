...
import { UploadCloud, FileImage, X, Plus, Trash2, Table, Download, Database, Webhook, Save, Lock, Unlock } from 'lucide-react'
...
export default function Uploads() {
    ...
    // Save / Confirm State
    type RecordStatus = 'draft' | 'saved' | 'confirmed'
    const [singleStatus, setSingleStatus] = useState<RecordStatus>('draft')
    const [singleConfirmedAt, setSingleConfirmedAt] = useState<string | null>(null)
    const [batchStatus, setBatchStatus] = useState<RecordStatus>('draft')
    const [batchConfirmedAt, setBatchConfirmedAt] = useState<string | null>(null)
    const [showUnlockDialog, setShowUnlockDialog] = useState(false)
    const [unlockTarget, setUnlockTarget] = useState<'single' | 'batch'>('single')
    ...
    const saveSingle = async () => {
        if (!basic.id || !basic.name) { showToast('warning', lang === 'ja' ? 'IDと氏名は必須です。' : 'ID and Name are required.'); return }
        setIsSubmitting(true); setSubmitProgress(30)
        await new Promise(r => setTimeout(r, 800))
        setSubmitProgress(100)
        setSingleStatus('saved')
        addAuditEntry({ userId: user?.id || '', userName: user?.fullName || '', action: 'save_draft', targetId: basic.id, targetName: basic.name })
        showToast('success', lang === 'ja' ? '一時保存しました' : 'Draft saved')
        setIsSubmitting(false); setSubmitProgress(0)
    }
    ...
    const confirmSingle = async () => {
        ...
        setSingleStatus('confirmed')
        setSingleConfirmedAt(now)
        addAuditEntry({ userId: user?.id || '', userName: user?.fullName || '', action: 'confirm', targetId: basic.id, targetName: basic.name })
        showToast('info', lang === 'ja' ? `スクリーニングを確定しました（${now}）` : `Screening confirmed (${now})`)
        setIsSubmitting(false); setSubmitProgress(0)
    }

    const submitSingle = async (e: React.FormEvent) => {
        e.preventDefault()
        // Form submit is now handled by the individual buttons
    }

    // Unlock confirmed record
    const requestUnlock = (target: 'single' | 'batch') => {
        if (!hasPermission('screening.edit')) {
            showToast('warning', lang === 'ja' ? 'この操作に必要な権限がありません' : 'Insufficient permissions for this action')
            return
        }
    ...
    const handleUnlockConfirm = (reason: string) => {
        const targetId = unlockTarget === 'single' ? basic.id : 'BATCH'
        addAuditEntry({ userId: user?.id || '', userName: user?.fullName || '', action: 'unlock_confirmed', targetId, reason })
        if (unlockTarget === 'single') {
            setSingleStatus('saved')
            setSingleConfirmedAt(null)
        } else {
            setBatchStatus('saved')
            setBatchConfirmedAt(null)
        }
        setShowUnlockDialog(false)
        showToast('info', lang === 'ja' ? '確定を解除しました。変更が可能です。' : 'Record unlocked. You may now edit.')
    }
                ...
                {activeTab === 'single' && (
                    <form onSubmit={submitSingle} className="space-y-6 animate-fade-in">
                        ...
                        <div className="panel p-5">
                            ...
                            {/* Status Bar */}
                            {singleStatus !== 'draft' && (
                                <div className={`status-bar ${singleStatus === 'confirmed' ? 'status-bar-confirmed' : 'status-bar-saved'}`} style={{ marginTop: 20 }}>
                                    {singleStatus === 'confirmed' ? <Lock size={18} /> : <Save size={18} />}
                                    <span style={{ flex: 1 }}>
                                        {singleStatus === 'confirmed'
                                            ? (lang === 'ja' ? `✓ 確定済み（${singleConfirmedAt}）` : `✓ Confirmed (${singleConfirmedAt})`)
                                            : (lang === 'ja' ? '✓ 一時保存済み — いつでも編集・確定が可能です' : '✓ Draft saved — editable anytime')}
                                    </span>
                                    {singleStatus === 'confirmed' && (
                                        <button type="button" className="btn btn-secondary" style={{ padding: '4px 12px', fontSize: '0.8rem', height: 28 }} onClick={() => requestUnlock('single')}>
                                            <Unlock size={14} /> {lang === 'ja' ? '変更を解除する' : 'Unlock'}
                                        </button>
                                    )}
                                </div>
                            )}
                            ...
                            <div style={{ marginTop: 24, display: 'flex', justifyContent: 'flex-end', gap: 16 }}>
                                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: singleStatus === 'confirmed' ? 'not-allowed' : 'pointer', marginRight: 'auto', opacity: singleStatus === 'confirmed' ? 0.5 : 1 }}>
                                    <input type="checkbox" checked={exam.urgent} onChange={e => { if (singleStatus !== 'confirmed') setExam({ ...exam, urgent: e.target.checked }) }} disabled={singleStatus === 'confirmed'} style={{ width: 18, height: 18 }} />
                                    <span style={{ color: exam.urgent ? '#ef4444' : 'var(--text)', fontWeight: exam.urgent ? 700 : 400 }}>{lang === 'ja' ? '緊急読影としてマーク' : 'Mark as Urgent'}</span>
                                </label>

                                <button type="button" className="btn btn-secondary" style={{ padding: '0 24px', height: 48, display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.95rem' }} onClick={saveSingle} disabled={isSubmitting || singleStatus === 'confirmed'}>
                                    <Save size={18} /> {lang === 'ja' ? '一時保存' : 'Save Draft'}
                                </button>
                                <button type="button" className="btn btn-primary" style={{ padding: '0 28px', height: 48, display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.95rem', background: '#3b82f6', border: 'none' }} onClick={confirmSingle} disabled={isSubmitting || singleStatus === 'confirmed'}>
                                    <Lock size={18} /> {lang === 'ja' ? '確定して送信' : 'Confirm & Submit'}
                                </button>
                            </div>
                            {isSubmitting && <div style={{ marginTop: 12, height: 4, background: 'var(--border)', borderRadius: 2, overflow: 'hidden' }}><div style={{ height: '100%', background: '#3b82f6', transition: 'all 0.3s', width: `${submitProgress}%` }} /></div>}
                        </div>
                    </form>
                )}
                    ...
                    <div className="space-y-6 animate-fade-in">
                        ...
                        {batchRows.length > 0 && (
                            <div className="panel animate-fade-in-up">
                                <div className="p-4 border-b border-gray-800 flex flex-col gap-3 bg-gray-900">
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        ...
                                        <div style={{ display: 'flex', gap: 8 }}>
                                            <button className="btn btn-secondary" onClick={async () => { setIsSubmitting(true); setSubmitProgress(30); await new Promise(r => setTimeout(r, 800)); setSubmitProgress(100); setBatchStatus('saved'); addAuditEntry({ userId: user?.id || '', userName: user?.fullName || '', action: 'batch_save', targetId: `BATCH-${batchRows.length}`, details: `${batchRows.length} rows` }); showToast('success', lang === 'ja' ? `一括保存しました（${batchRows.length}件）` : `Batch saved (${batchRows.length} items)`); setIsSubmitting(false); setSubmitProgress(0) }} disabled={isSubmitting || batchStatus === 'confirmed'} style={{ padding: '0 16px', height: 36, display: 'flex', alignItems: 'center', gap: 6 }}>
                                                <Save size={16} /> {lang === 'ja' ? '一括保存' : 'Save All'}
                                            </button>
                                            <button className="btn btn-primary" onClick={async () => { setIsSubmitting(true); setSubmitProgress(30); await new Promise(r => setTimeout(r, 1500)); setSubmitProgress(100); const now = new Date().toLocaleString('ja-JP'); setBatchStatus('confirmed'); setBatchConfirmedAt(now); addAuditEntry({ userId: user?.id || '', userName: user?.fullName || '', action: 'batch_confirm', targetId: `BATCH-${batchRows.length}`, details: `${batchRows.length} rows` }); showToast('info', lang === 'ja' ? `一括確定しました（${now}）` : `Batch confirmed (${now})`); setIsSubmitting(false); setSubmitProgress(0) }} disabled={isSubmitting || batchStatus === 'confirmed'} style={{ padding: '0 16px', height: 36, display: 'flex', alignItems: 'center', gap: 6, background: '#3b82f6', border: 'none' }}>
                                                <Lock size={16} /> {lang === 'ja' ? '一括確定' : 'Confirm All'}
                                            </button>
                                        </div>
                                    </div>
                                    {/* Batch Status Bar */}
                                    {batchStatus !== 'draft' && (
                                        <div className={`status-bar ${batchStatus === 'confirmed' ? 'status-bar-confirmed' : 'status-bar-saved'}`}>
                                            {batchStatus === 'confirmed' ? <Lock size={16} /> : <Save size={16} />}
                                            <span style={{ flex: 1, fontSize: '0.85rem' }}>
                                                {batchStatus === 'confirmed'
                                                    ? (lang === 'ja' ? `✓ 一括確定済み（${batchConfirmedAt}）` : `✓ Batch confirmed (${batchConfirmedAt})`)
                                                    : (lang === 'ja' ? '✓ 一括保存済み' : '✓ Batch saved')}
                                            </span>
                                            {batchStatus === 'confirmed' && (
                                                <button className="btn btn-secondary" style={{ padding: '4px 10px', fontSize: '0.75rem', height: 26 }} onClick={() => requestUnlock('batch')}>
                                                    <Unlock size={12} /> {lang === 'ja' ? '変更を解除' : 'Unlock'}
                                                </button>
                                            )}
                                        </div>
                                    )}
    ...
    },
    {
      ...
      "request": { "method": "POST", "url": "DiagnosticReport" }
            ...
            <ConfirmDialog
                ...
                title={lang === 'ja' ? '確定済みの内容を変更しますか？' : 'Unlock confirmed record?'}
                message={lang === 'ja'
                    ? '確定済みのデータを編集するには、変更理由の入力が必要です。この操作は操作ログに記録されます。'
                    : 'To edit confirmed data, you must provide a reason. This action will be logged.'}
                requireReason={true}
                reasonLabel={lang === 'ja' ? '変更理由' : 'Reason for change'}
                reasonPlaceholder={lang === 'ja' ? '変更の理由を入力してください...' : 'Enter reason for change...'}
                confirmText={lang === 'ja' ? '変更を許可する' : 'Allow Changes'}
                cancelText={lang === 'ja' ? 'キャンセル' : 'Cancel'}
                variant="warning"
